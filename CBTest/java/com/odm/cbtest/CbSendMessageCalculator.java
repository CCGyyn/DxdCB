...
public class CbSendMessageCalculator {
    ...
    public CbSendMessageCalculator( final Context context,
             final List<CbGeoUtils.Geometry> fences) {
        this(context, fences, context.getResources().getInteger(R.integer.geo_fence_threshold));
    }

    public CbSendMessageCalculator( final Context context,
             final List<CbGeoUtils.Geometry> fences, final double thresholdMeters) {
    ...
    private int calculateSysSingleFence(CbGeoUtils.LatLng coordinate, double accuracyMeters,
        ...
        Optional<Double> maybeDistance =
                CbGeoUtilsApp.distance(fence, coordinate);
        Log.d("maybeDistance.isPresent=" + maybeDistance.isPresent());
        if (!maybeDistance.isPresent()) {
            return SEND_MESSAGE_ACTION_DONT_SEND;
        }

        double distance = maybeDistance.get();
        Log.d("accuracyMeters=" + accuracyMeters + ", distance=" + distance + ", mThresholdMeters=" + mThresholdMeters);
